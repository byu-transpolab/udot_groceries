...
\begin{abstract}
Convenient access to nutrition resources is a critical element of public
...
The ability of people to access nutrition resources has been studied at
...
At the same time, access to nutrition resources and to community
...
health researchers have constructed several quantitative measures. These
measures include presence of a store --- or counts of stores --- within
a travel time limit by a particular mode, or the distance to the nearest
...
farther are people willing to travel to go to a store that is cheaper or
...
These data are linked through simulation to develop a simultaneous
multinomial logit mode and destination choice model that measures
variables significant to grocery store choice in Utah. These models are
then used to measure neighborhood-level accessibility to stores and
evaluate potential transportation, land use, and related policies.
...
disparities in access to nutrition resources with observed spatial
disparities in economic resources or health indicators including obesity
rates, caloric intake, and other outcomes. Complete --- though somewhat
dated --- reviews of this literature can be had from Beaulac et al.
(2009) and Walker et al. (2010). Park et al. (2025) present a more
...
The existing nutrition environment literature has been somewhat
...
measures of spatial access. Cetateanu et al. (2016) and Widener et al.
...
Measures of the nutrition environment beyond spatial proximity are
similarly inconsistent, though some validated methods have been
developed (Baker et al., 2024). A primary obstacle in developing such
...
(Chen et al., 2016; Cooksey-Stowers et al., 2017). Foremost among the
...
included and which are to be excluded. For example, Widener et al.
(2018) develop a complex time-and-activity space measure respondent's
...
researchers to assert what they will include and what they will exclude.

What has not been pursued in the literature to this point is an approach
...
nutrition resources \emph{as well as} the availability and quality of
...
and access to urban green spaces (Macfarlane et al., 2021) and to better
inform parks-related policy analysis (Macfarlane et al., 2022). It is
...
Valley City. The housing density and locations of stores in each
community are shown in Figure~\ref{fig-communities}, and the context of
each community in Utah is shown in Figure~\ref{fig-community-context}.
...
\caption{\label{fig-community-context}Context of communities in
...
cost of foods identified as more healthful within the NEMS-S framework,
...
Within the NEMS-S framework, ``healthful'' options are operationalized
...
particular items as well as NEMS-S defined more healthful options; and a
...
healthful options available in the store as defined by the NEEMS-S
framework (i.e., low-calorie chips or low-fat milk). If the store does
...
the price spread between healthful and less healthful options as defined
by the NEMS-S framework: if the price of whole wheat bread is cheaper
...
towards those options. Accordingly, higher scores reflect greater
...
contains a certain amount of poultry, but the NEMS-S instrument collects
...
transportation-focused and nutrition environment-focused improvements.
It is therefore essential that the measure of travel impedance
incorporate travel by multiple modes of transportation, so that highway
improvements and transit or active transport improvements can be
compared.

Just as the log-sum of a destination choice model is a measure that sums
the utility of multiple destination attributes and costs, the log-sum of
a mode choice model is the expected maximum utility of all available
travel modes, weighted by preferences and costs (Ben-Akiva \& Lerman,
1985). In this study we assert the following mode choice utility
...
departure between 8 AM and 9 AM on May 10, 2023; this period on a
Tuesday morning in the AM peak provides the highest level of service
possible in the transit system. The total allowable trip time by any
mode was set to 120 minutes, and the walk distance was capped at 10
kilometers, mirroring limits set in the regional travel model. If a
...
constructed to illustrate potential applications of a utility-based
model, and not as an evaluation of proposed policies. We first describe
...
and the total access benefit is defined as the consumer surplus given in
...
to increase their utility by one unit. In actuality, the utility formula
in Equation~\ref{eq-scenu} is a relative utility added by an unknown
constant, \(U_{ij} = f(\mathbf{\beta}, X_{ij}) + C\), but this \(C\)
term is included in all the alternatives and therefore cancels out in
estimation (Train, 2009). This means we cannot assess the
...
availability of quality goods. In response, some public agencies and
...
et al., 2018). To examine the potential for this strategy to improve
...
dollar store. In this scenario, we improve the attributes of an existing
ethnic store near 2700 W 3500 S in West Valley City.
...
\subcaption{\label{fig-scenario-results-1}New full-service store}
...
\subcaption{\label{fig-scenario-results-2}Improved existing store}
...
\subcaption{\label{fig-scenario-results-3}Improved non-motorized and
transit access}
...
\subcaption{\label{fig-scenario-results-4}Expanded grocery delivery}

\end{minipage}%

\caption{\label{fig-scenario-results}Estimated per-household benefits
under the four Salt Lake Valley scenarios.}

\end{sidewaysfigure}%

Figure~\ref{fig-scenario-results} shows the geographic distribution of
benefits associated with each Salt Lake Valley scenario. The benefits
from locating a new store at a site in the Salt Lake community
(Figure~\ref{fig-scenario-results-1}) are largest immediately next to
the new store, where they exceed \(3\) for each household each time the
household makes a trip to a grocery store. The improved-store scenario
(Figure~\ref{fig-scenario-results-2}) is similar in shape, but the scale
...
geographic region. The transit and non-motorized access scenario
...
is shown in color, but the reader should note that the scale of benefits
...
the four scenarios The Households column multiplies the difference in
...
magnitude higher than the cost of a single new grocery store. It is
acknowledged that improving non-automobile infrastructure and services
would have benefits for all trip types --- beyond just the grocery store
trips --- that we do not attempt to enumerate here. It is also not clear
...
achieve profitability later (D. V. Moore, 2022). Resolving all of these
...
access have perhaps limited efforts at developing solutions. In this
research, a model of nutrition resource access that includes attributes
...
barrier. These findings may not be general, but they largely support the
...
The broad literature of attempts to evaluate the nutrition environment
...
the nutrition environment literature. We acknowledge that NEMS-S
...
diet-related healthfulness. The NEMS-S focus on low-calorie and low-fat
...
This research implemented travel impedance measures calculated using an
...
\bibitem[\citeproctext]{ref-ansai2023}
Ansai, N., Wambogo, E. A., Herrick, K. A., Zimmer, M., Reedy, J., Hales,
C. M., \& Ogden, C. L. (2023). Dietary contributions of food outlets by
urbanization level in the {US} population aged 2 years and
older---{NHANES} 2013--2018. \emph{The American Journal of Clinical
Nutrition}, \emph{117}(5), 946--954.
...
Baker, K., Burd, L., \& Figueroa, R. (2024). Consumer nutrition
environment measurements for nutrient-dense food availability and food
sustainability: A scoping review. \emph{Archives of Public Health},
\emph{82}(1), 7. \url{https://doi.org/10.1186/s13690-023-01231-y}
...
\bibitem[\citeproctext]{ref-bennion2022}
Bennion, N., Redelfs, A. H., Spruance, L., Benally, S., \& Sloan-Aagard,
C. (2022). Driving {Distance} and {Food Accessibility}: {A Geospatial
Analysis} of the {Food Environment} in the {Navajo Nation} and {Border
Towns}. \emph{Frontiers in Nutrition}, \emph{9}, 904119.
...
\bibitem[\citeproctext]{ref-burns2007}
Burns, C. M., \& Inglis, A. D. (2007). Measuring food access in
{Melbourne}: {Access} to healthy and fast foods by car, bus and foot in
an urban municipality in {Melbourne}. \emph{Health \& Place},
\emph{13}(4), 877--885.
\url{https://doi.org/10.1016/j.healthplace.2007.02.005}

\bibitem[\citeproctext]{ref-cetateanu2016}
Cetateanu, A., Luca, B.-A., Popescu, A. A., Page, A., Cooper, A., \&
Jones, A. (2016). A novel methodology for identifying environmental
exposures using {GPS} data. \emph{International Journal of Geographical
Information Science}, \emph{30}(10), 1944--1960.
\url{https://doi.org/10.1080/13658816.2016.1145682}

\bibitem[\citeproctext]{ref-charreire2010}
Charreire, H., Casey, R., Salze, P., Simon, C., Chaix, B., Banos, A.,
...
review. \emph{Public Health Nutrition}, \emph{13}(11), 1773--1785.
...
\bibitem[\citeproctext]{ref-economicresearchservice2025}
Economic Research Service. (2025). \emph{Food {Access Research Atlas} -
{Documentation}}.
...
\bibitem[\citeproctext]{ref-glanz2023}
Glanz, K., Fultz, A. K., Sallis, J. F., Clawson, M., McLaughlin, K. C.,
Green, S., \& Saelens, B. E. (2023). Use of the {Nutrition Environment
Measures Survey}: {A Systematic Review}. \emph{American Journal of
Preventive Medicine}, \emph{65}(1), 131--142.
...
Haboush-Deloye, A. L., Knight, M. A., Bungum, N., \& Spendlove, S.
(2023). Healthy foods in convenience stores: Benefits, barriers, and
best practices. \emph{Health Promotion Practice}, \emph{24}(1\_suppl),
...
\bibitem[\citeproctext]{ref-jaskiewicz2016}
Jaskiewicz, L., Block, D., \& Chavez, N. (2016). Finding {Food Deserts}:
{A Comparison} of {Methods Measuring Spatial Access} to {Food Stores}.
\emph{Health Promotion Practice}, \emph{17}(3), 400--407.
...
\bibitem[\citeproctext]{ref-kostopoulou2024}
Kostopoulou, E., Christofa, E., Gonzales, E., \& Krevat, D. (2024).
Measuring and modeling food accessibility by transportation mode.
\emph{Journal of Transport Geography}, \emph{118}, 103907.
...
\bibitem[\citeproctext]{ref-macfarlane2021modeling}
Macfarlane, G. S., Boyd, N., Taylor, J. E., \& Watkins, K. (2021).
Modeling the impacts of park access on health outcomes: A utility-based
accessibility approach. \emph{Environment and Planning B: Urban
Analytics and City Science}, \emph{48}(8), 2289--2306.
...
\bibitem[\citeproctext]{ref-moore2008}
Moore, L. V., Diez Roux, A. V., \& Brines, S. (2008). Comparing
{Perception-Based} and {Geographic Information System} ({GIS})-{Based
Characterizations} of the {Local Food Environment}. \emph{Journal of
Urban Health}, \emph{85}(2), 206--216.
\url{https://doi.org/10.1007/s11524-008-9259-x}

\bibitem[\citeproctext]{ref-murphy2017}
Murphy, M., Koohsari, M. J., Badland, H., \& Giles-Corti, B. (2017).
...
\emph{Food {Deserts} in {Context}: {How Site-Specific Factors Reshape
Food Deserts Discussion}} (\{\{SSRN Scholarly Paper\}\} 5632970). Social
...
\bibitem[\citeproctext]{ref-rhone2017}
Rhone, A., Ver Ploeg, M., Dicken, C., Williams, R., \& Breneman, V.
(2017). \emph{Low-{Income} and {Low-Supermarket-Access Census Tracts},
2010-2015} (Economic {Information Bulletin Number} 165).
...
\bibitem[\citeproctext]{ref-widener2018}
Widener, M. J., Minaker, L. M., Reid, J. L., Patterson, Z., Ahmadi, T.
...
young urban adults in {Canada}. \emph{Public Health Nutrition},
\emph{21}(11), 2103--2116.
...
Young, S., DeNomie, M., Sabir, J., Gass, E., \& Tobin, J. (2018). Around
the corner to better health: A milwaukee corner store initiative.
\emph{American Journal of Health Promotion}, \emph{32}(6), 1353--1356.
