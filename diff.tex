...
%DIF ADD Where-s-dinner-coming-from--A-utility-based-investigation-of-access-to-nutrition-in-Utah..tex   Mon May 18 20:30:58 2026
...
\begin{abstract}
Convenient access to \DIFdelbegin \DIFdel{high-quality nutrition }\DIFdelend \DIFaddbegin \DIFadd{nutrition resources }\DIFaddend is a critical element of public
health as well as an important interface between communities and the
transportation system. In this research, we seek to construct a detailed
picture of the nutrition environment in three communities in Utah,
alongside the community members' ability to access that environment
through multiple transportation modes. In doing so we construct a
utility-based accessiblity model enabled by modern mobility device data.
This model reveals the tradeoffs between the quality and price of goods
on one hand and the distance traveled to reach them on the other. We
then apply this model to a series of potential access-improving
policies: building a new store, improving an existing store, and
improving the non-automobile transport network between residents and
...
transportation system, at likely lower costs. The findings suggest that
transportation agencies work to increase the availability of
...
The ability of people to access \DIFdelbegin \DIFdel{quality nutrition }\DIFdelend \DIFaddbegin \DIFadd{nutrition resources }\DIFaddend has been studied at
...
At the same time, access to nutrition \DIFaddbegin \DIFadd{resources }\DIFaddend and to community
resources in general is frustratingly hard to define. ``Accessibility''
is an abstract concept without a specific quantitative definition (Handy
...
health researchers have constructed several quantitative measures\DIFdelbegin \DIFdel{, such
as }\DIFdelend \DIFaddbegin \DIFadd{. These
measures include }\DIFaddend presence of a store \DIFaddbegin \DIFadd{--- or counts of stores --- }\DIFaddend within
a travel time \DIFdelbegin \DIFdel{buffer}\DIFdelend \DIFaddbegin \DIFadd{limit by a particular mode}\DIFaddend , or the distance to the nearest
store. These measures are relatively easy to calculate using readily
available GIS software, but elide much useful information (Dong et al.,
2006; Logan et al., 2019). These types of measures require the
...
\DIFdelbegin \DIFdel{further }\DIFdelend \DIFaddbegin \DIFadd{farther }\DIFaddend are people willing to travel to go to a store that is cheaper or
...
\tightlist
\item
  A detailed survey of the nutrition market in three Utah communities\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{,
  operationalized using the NEMS-S framework (Glanz et al., 2007).
}\DIFaddend \item
...
These data \DIFdelbegin \DIFdel{will be combined in order to develop accurate logit models
that demonstrate the variables that are }\DIFdelend \DIFaddbegin \DIFadd{are linked through simulation to develop a simultaneous
multinomial logit mode and destination choice model that measures
variables }\DIFaddend significant to grocery store choice in Utah. These models \DIFdelbegin \DIFdel{could then be used to find }\DIFdelend \DIFaddbegin \DIFadd{are
then used to measure neighborhood-level }\DIFaddend accessibility to stores and
\DIFdelbegin \DIFdel{impact transportationpolicy to improve quality of life for
all communities in Utah}\DIFdelend \DIFaddbegin \DIFadd{evaluate potential transportation, land use, and related policies}\DIFaddend .
...
\section{\DIFdelbegin \DIFdel{Methods}\DIFdelend \DIFaddbegin \DIFadd{Literature Review}\DIFaddend }\DIFdelbegin %DIFDELCMD < \label{sec-methods}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{sec-literature}
\DIFaddend 
...
\DIFaddbegin \DIFadd{disparities in }\DIFaddend access to nutrition \DIFdelbegin \DIFdel{with health outcomes including obesity
}\DIFdelend \DIFaddbegin \DIFadd{resources with observed spatial
disparities in economic resources or health indicators including obesity
rates}\DIFaddend , caloric intake, and \DIFdelbegin \DIFdel{the like}\DIFdelend \DIFaddbegin \DIFadd{other outcomes}\DIFaddend . Complete --- though somewhat
dated --- reviews of this literature can be had from \DIFdelbegin \DIFdel{(}\DIFdelend Beaulac et al.
\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend 2009) and \DIFdelbegin \DIFdel{(}\DIFdelend Walker et al. \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend 2010). \DIFdelbegin \DIFdel{More recent work has extended the description and refinement
of the measures used to evaluate food accessand control for confounding
variables. (Widener \& Shannon, 2014) considered that temporal access to quality food is as important as spatial access }\DIFdelend \DIFaddbegin \DIFadd{Park et al. (2025) present a more
recent systematic review of this literature, aided by an large language
model-informed analysis of more than 800 articles; this more recent
review emphasizes the wide variety of measures and methods that have
been applied to the problem, with a focus on differences between global
regions.
}

\DIFadd{The existing nutrition environment literature has been somewhat
inconsistent in its application of spatial or travel impedance units.
Charreire et al. (2010) provides a categorization of measures in the
literature to that point: almost all studies used Euclidean or
occasionally network distance buffers, most frequently of one kilometer;
a few studies used network travel time by an assumed travel mode, kernel
density (L. V. Moore et al., 2008), or spatial clustering. In the last
few years, a number of studies have attempted to explore more complex
measures of spatial access}\DIFaddend . \DIFdelbegin \DIFdel{(Aggarwal }\DIFdelend \DIFaddbegin \DIFadd{Cetateanu et al. (2016) and Widener et al.
(2018) each pursue measures of time and space described by a person's
observed activity pattern and use it to explain food retail and
purchasing behaviors. These measures have not entered policy analysis at
scale, however. Public health agencies in the United States continue to
use Euclidean distance to the nearest store as the basis for policy
analysis (Economic Research Service, 2025; Rhone et al., 2017). When
using travel times, the literature most often selects a single mode of
travel, usually car or walking (Bennion et al., 2022). When researchers
consider multiple modes, it is usually done as a comparison (Burns \&
Inglis, 2007; Kostopoulou et al., 2024) or as multiple independent
covariates in a large model (Murphy et al., 2017) rather than as a
combined measure. This difference in methodological approaches matters
substantially: Jaskiewicz et al. (2016) found little agreement in
definitions of good and poor food access between six different measures
of access in Chicago.
}

\DIFadd{Measures of the nutrition environment beyond spatial proximity are
similarly inconsistent, though some validated methods have been
developed (Baker }\DIFaddend et al., \DIFdelbegin \DIFdel{2014)suggested that spatial access was not as important as store choice,
given that most people were not observed to shop at the nearest
vendor.By contrast, }\DIFdelend \DIFaddbegin \DIFadd{2024). A primary obstacle in developing such
measures is the complexity of the nutrition environment; many people
obtain food from widely varying sources including convenience stores,
supermarkets, small grocery stores, and restaurants of all kinds (Ansai
et al., 2023). Non-spatial measures of this environment are important
for policy analysis given that many people do not shop at their nearest
store alone (Aggarwal et al., 2014), and that the quality of offerings
at stores may explain obesity rates as much as spatial access to stores
}\DIFaddend (Chen et al., 2016\DIFdelbegin \DIFdel{) compared spatial access to quality food
vendors with observed food expenditures and showed poor access explained
obesity even when controlling for consumption. (}\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend Cooksey-Stowers \DIFaddbegin \DIFadd{et al., 2017). Foremost among the
validated measures of the nutrition environment is the NEMS-S framework
(Glanz et al., 2007; Glanz et al., 2023), in which trained surveyors
record the presence, price, and shelf space allocated to various food
items in stores. But even the NEMS-S is not entirely comprehensive:
separate instruments exist for stores (NEMS-S), corner stores (NEMS-CS),
and restaurants (NEMS-R), vending machines (NEMS-V), and quick service
prepared food stores (NEMS-GG). Applying one measure to the entire
spatial environment is not a viable approach, and may result in
discontinuities. As a result of these discontinuities, researchers
frequently segment the environment, asserting which stores are to be
included and which are to be excluded. For example, Widener }\DIFaddend et al.
\DIFdelbegin \DIFdel{,
2017) jointly pursued spatial proximitywith quality of
offerings and
showed the latter might be more predictive of
obesity rates}\DIFdelend \DIFaddbegin \DIFadd{(2018) develop a complex time-and-activity space measure respondent's
spatial proximity, but apply this measure to discrete segments of
grocery stores, convenience stores, etc. rather than to a holistic
picture of the nutrition environment. More simplistic measures of
spatial access --- e.g., Euclidean distance to nearest store --- require
researchers to assert what they will include and what they will exclude}\DIFaddend .

What has not been \DIFdelbegin \DIFdel{frequently attempted in the nutrition access
...
between the two. }\DIFdelend \DIFaddbegin \DIFadd{pursued in the literature to this point is an approach
that considers how the multi-modal travel behavior of individuals with
respect to transportation infrastructure shapes spatial access to
nutrition resources }\emph{\DIFadd{as well as}} \DIFadd{the availability and quality of
foods at the relevant destinations. Such an approach has been shown to
improve the correlation between spatial disparities in health outcomes
and access to urban green spaces }\DIFaddend (\DIFdelbegin \textbf{\DIFdel{macfarlane2021b?}}%DIFAUXCMD
\DIFdel{) illustrated the potential
...
into a policy analysis of park space during Covid-19 (}\DIFdelend Macfarlane et al., \DIFdelbegin \DIFdel{2022) . The potential for application of this methodology to the
nutrition literature is well-motivated by the previous attempts as well
as the lack of clear policy solutions (Wright }\DIFdelend \DIFaddbegin \DIFadd{2021) and to better
inform parks-related policy analysis (Macfarlane }\DIFaddend et al., \DIFdelbegin \DIFdel{2016). }\DIFdelend \DIFaddbegin \DIFadd{2022). It is
this combination that we pursue in this study.
}\DIFaddend 

\DIFaddbegin \bookmarksetup{startatroot}

\section{\DIFadd{Methods}}\label{sec-methods}

\DIFaddend This section describes how we construct a model of access to grocery
...
not new, and the theory for doing so is described in \DIFdelbegin \DIFdel{(}\DIFdelend Ben-Akiva \& Lerman
\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend 1985, p. 301). Effectively, the natural logarithm of the denominator in
...
\DIFdelbegin \DIFdel{(}\DIFdelend Macfarlane et al. \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend 2022), relying on commercial location-based services
...
Valley City. The \DIFdelbegin \DIFdel{communities }\DIFdelend \DIFaddbegin \DIFadd{housing density and locations of stores in each
community }\DIFaddend are shown in \DIFdelbegin \DIFdel{a wider context in
Figure~\ref{fig-communities}}\DIFdelend \DIFaddbegin \DIFadd{Figure~\ref{fig-communities}, and the context of
each community in Utah is shown in Figure~\ref{fig-community-context}}\DIFaddend .

\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{sidewaysfigure}
\DIFaddendFL 
...
\caption{\label{fig-communities}Location of \DIFdelbeginFL \DIFdelFL{study regions }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{surveyed stores }\DIFaddendFL in \DIFaddbeginFL \DIFaddFL{three
comunities in }\DIFaddendFL Utah\DIFaddbeginFL \DIFaddFL{, with housing density at block group level and size
of store measured by number of registers}\DIFaddendFL .}

\DIFaddbeginFL \end{sidewaysfigure}%DIF > 

\begin{figure}

\centering{

\includegraphics{03_methods_files/figure-pdf/fig-community-context-1.pdf}

}

\caption{\label{fig-community-context}\DIFaddFL{Context of communities in
Figure~\ref{fig-communities} within the state of Utah.}}
...
cost of \DIFdelbegin \DIFdel{healthy foods in an environment}\DIFdelend \DIFaddbegin \DIFadd{foods identified as more healthful within the NEMS-S framework}\DIFaddend ,
and has been validated for this purpose. Beyond superficial attributes
such as the store category (dollar store, convenience store, ethnic
market, etc.) and the number of registers, the NEMS-S collects detailed
information about numerous store offerings such as the availability of
produce, dairy products, lean meats, juices, and canned and dry goods of
various specific types. Of particular interest to the survey are
availability and price differentials of lower-fat alternatives: for
example, the survey instrument requests the shelf space allocated to
milk products of various fat levels and the price of each product.
\DIFaddbegin \DIFadd{Within the NEMS-S framework, ``healthful'' options are operationalized
primarily through the availability and relative pricing of lower-fat and
lower-calorie alternatives; NEMS-S captures one dimension of
diet-related healthfulness but does not represent a comprehensive
assessment of nutritional quality across all populations.
}\DIFaddend 
...
particular items as well as \DIFdelbegin \DIFdel{lower-calorie }\DIFdelend \DIFaddbegin \DIFadd{NEMS-S defined more healthful }\DIFaddend options; and a
cost score describing the spread between prices of these options. These
score values are described in \DIFdelbegin \DIFdel{(}\DIFdelend Lunsford et al. \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend 2021), and we developed
an R package to compute the scores; this package is available at
...
healthful options available in the store \DIFdelbegin \DIFdel{, such as }\DIFdelend \DIFaddbegin \DIFadd{as defined by the NEEMS-S
framework (i.e., }\DIFaddend low-calorie chips \DIFdelbegin \DIFdel{, }\DIFdelend or low-fat milk\DIFaddbegin \DIFadd{)}\DIFaddend . If the store does
not have a more healthful option in a category it receives a lower
score, so stores with more availability of healthful food options will
receive a higher availability score. For the cost score, the measure is
the price spread between healthful and less healthful options \DIFaddbegin \DIFadd{as defined
by the NEMS-S framework}\DIFaddend : if the price of whole wheat bread is cheaper
than white bread, the store receives positive points for the cost
option, if the price is the same then zero points are awarded, and if
the wheat bread is more expensive then the store receives negative
points. Thus a store with a higher availability and cost score will have
both more healthful options, and a more advantageous pricing scheme
towards those options. \DIFaddbegin \DIFadd{Accordingly, higher scores reflect greater
availability and more favorable pricing of these NEMS-S defined
healthful options, rather than a comprehensive measure of overall
nutritional quality.
}\DIFaddend 
...
the approach of \DIFdelbegin \DIFdel{(}\DIFdelend Hedrick et al. \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend 2022). This market basket score is based
...
contains a certain amount of poultry, but the NEMS-S \DIFdelbegin \DIFdel{score
}\DIFdelend \DIFaddbegin \DIFadd{instrument }\DIFaddend collects
the per-pound cost of ground beef at various fat contents. For any
stores that were missing any of the elements in the market basket, we
first substituted with another ingredient that would fit the nutritional
...
type, \DIFaddbegin \DIFadd{information }\DIFaddend which we also collected in the manual data assembly
...
distances). \DIFdelbegin %DIFDELCMD < 
...
%DIFDELCMD < \includegraphics[width=5in,height=\textheight]{03_methods_files/figure-pdf/fig-marketimp-1.pdf}
%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{fig-marketimp}%%%
\DIFdelFL{Imputed market price values for 12 random
grocery stores.}}
%DIFAUXCMD
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelend Thirty iterations of the multiple imputation algorithm were
run for each of ten independent imputations. \DIFdelbegin \DIFdel{Figure~\ref{fig-marketimp} shows the
...
still predicts a wide range of possible prices for most stores. }\DIFdelend When using the imputed data
...
\DIFdelbegin \DIFdel{infrastructure-focused and }\DIFdelend \DIFaddbegin \DIFadd{transportation-focused and nutrition }\DIFaddend environment-focused improvements\DIFdelbegin \DIFdel{to the
nutrition access of households}\DIFdelend .
It is therefore essential that \DIFdelbegin \DIFdel{we use a
travel impedance measure that can combine and compare the cost of traveling }\DIFdelend \DIFaddbegin \DIFadd{the measure of travel impedance
incorporate travel }\DIFaddend by multiple modes \DIFaddbegin \DIFadd{of transportation, }\DIFaddend so that highway
improvements and transit \DIFdelbegin \DIFdel{/
}\DIFdelend \DIFaddbegin \DIFadd{or }\DIFaddend active transport improvements can be
compared\DIFdelbegin \DIFdel{in the same basic model}\DIFdelend .

Just as the log-sum of a destination choice model is a measure that sums
the utility of multiple destination attributes and costs\DIFdelbegin \DIFdel{in a rigorous
manner}\DIFdelend , the log-sum of
a mode choice model \DIFdelbegin \DIFdel{combines the utilities }\DIFdelend \DIFaddbegin \DIFadd{is the expected maximum utility }\DIFaddend of all available
travel modes\DIFaddbegin \DIFadd{, weighted by preferences and costs (Ben-Akiva \& Lerman,
1985)}\DIFaddend . In this study we assert the following mode choice utility
...
departure between 8 AM and 9 AM on May 10, \DIFdelbegin \DIFdel{2023. }\DIFdelend \DIFaddbegin \DIFadd{2023; this period on a
Tuesday morning in the AM peak provides the highest level of service
possible in the transit system. }\DIFaddend The total allowable trip time by any
mode was set to 120 minutes, and the walk distance was capped at 10
kilometers\DIFdelbegin \DIFdel{; if }\DIFdelend \DIFaddbegin \DIFadd{, mirroring limits set in the regional travel model. If }\DIFaddend a
particular \(i,j\) pair exceeded these parameters then the mode was
presumed to not be available and contributes no utility to the log-sum.
...
store. \DIFdelbegin \DIFdel{(}\DIFdelend Macfarlane et al. \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend 2022) present a method for estimating
...
%DIFDELCMD < \toprule
%DIFDELCMD <   & Access & NEMS & Attributes & All\\
%DIFDELCMD < \midrule
%DIFDELCMD < Mode Choice Log-sum & 8.063** &  &  & 8.686**\\
%DIFDELCMD <  & (95.356) &  &  & (87.770)\\
%DIFDELCMD < NEMS Availability Score &  & 0.032** &  & -0.024**\\
%DIFDELCMD <  &  & (22.020) &  & (-7.369)\\
%DIFDELCMD < NEMS Cost Score &  & 0.035** &  & 0.041**\\
%DIFDELCMD <  &  & (7.688) &  & (5.946)\\
%DIFDELCMD < USDA Market Basket &  &  & -0.007** & -0.008**\\
%DIFDELCMD <  &  &  & (-9.814) & (-8.855)\\
%DIFDELCMD < Registers &  &  & 0.056** & 0.065**\\
%DIFDELCMD <  &  &  & (54.586) & (41.357)\\
%DIFDELCMD < Store Type: Dollar Store &  &  & 1.948** & 2.086**\\
%DIFDELCMD <  &  &  & (56.866) & (41.000)\\
%DIFDELCMD < Store Type: Convenience Store &  &  & -1.990** & -2.422**\\
%DIFDELCMD <  &  &  & (-7.400) & (-8.495)\\
%DIFDELCMD < Store Type: Other &  &  & -1.785** & -1.813**\\
%DIFDELCMD <  &  &  & (-12.641) & (-11.991)\\
%DIFDELCMD < \midrule
%DIFDELCMD < AIC & 31,874.44 & 49,051.55 & 41,985.6 & 25,402.43\\
%DIFDELCMD < \$\textbackslash{}rho\textasciicircum{}2\_0\$ & 0.359 & 0.013 & 0.155 & 0.489\\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \multicolumn{5}{l}{\rule{0pt}{1em}* p $<$ 0.05, ** p $<$ 0.01}\\
%DIFDELCMD < \multicolumn{5}{l}{\rule{0pt}{1em}t-statistics in parentheses}\\
...
$\rho_0^2$ & 0.359 & 0.013 & 0.155 & 0.489\\
\bottomrule
...
%DIFDELCMD <   & Access & NEMS & Attributes & All\\
%DIFDELCMD < \midrule
%DIFDELCMD < Mode Choice Log-sum & 9.870** &  &  & 12.044**\\
%DIFDELCMD <  & (74.095) &  &  & (74.423)\\
%DIFDELCMD < NEMS Availability Score &  & 0.129** &  & 0.002\\
%DIFDELCMD <  &  & (71.898) &  & (0.580)\\
%DIFDELCMD < NEMS Cost Score &  & -0.036** &  & 0.057**\\
%DIFDELCMD <  &  & (-7.857) &  & (9.607)\\
%DIFDELCMD < USDA Market Basket &  &  & -0.010** & -0.006**\\
%DIFDELCMD <  &  &  & (-13.065) & (-6.953)\\
%DIFDELCMD < Registers &  &  & 0.104** & 0.129**\\
%DIFDELCMD <  &  &  & (70.019) & (52.110)\\
%DIFDELCMD < Store Type: Dollar Store &  &  & 0.325** & 0.494**\\
%DIFDELCMD <  &  &  & (7.250) & (8.943)\\
%DIFDELCMD < Store Type: Other &  &  & 0.248* & 0.556**\\
%DIFDELCMD <  &  &  & (2.211) & (4.614)\\
%DIFDELCMD < \midrule
%DIFDELCMD < AIC & 42,820.66 & 42,541.44 & 40,195.01 & 31,863.31\\
%DIFDELCMD < \$\textbackslash{}rho\textasciicircum{}2\_0\$ & 0.138 & 0.144 & 0.191 & 0.359\\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \multicolumn{5}{l}{\rule{0pt}{1em}* p $<$ 0.05, ** p $<$ 0.01}\\
%DIFDELCMD < \multicolumn{5}{l}{\rule{0pt}{1em}t-statistics in parentheses}\\
%DIFDELCMD < \end{tabular}
...
$\rho_0^2$ & 0.138 & 0.144 & 0.191 & 0.359\\
\bottomrule
\multicolumn{5}{l}{\rule{0pt}{1em}* p $<$ 0.05, ** p $<$ 0.01}\\
\multicolumn{5}{l}{\rule{0pt}{1em}t-statistics in parentheses}\\
\end{tabular}
...
%DIFDELCMD <   & Access & NEMS & Attributes & All\\
%DIFDELCMD < \midrule
%DIFDELCMD < Mode Choice Log-sum & 0.709** &  &  & 1.205**\\
%DIFDELCMD <  & (81.466) &  &  & (73.616)\\
%DIFDELCMD < NEMS Availability Score &  & 0.139** &  & 0.065**\\
%DIFDELCMD <  &  & (63.869) &  & (12.857)\\
%DIFDELCMD < NEMS Cost Score &  & 0.227** &  & 0.055**\\
%DIFDELCMD <  &  & (35.386) &  & (6.721)\\
%DIFDELCMD < USDA Market Basket &  &  & -0.011** & -0.033**\\
%DIFDELCMD <  &  &  & (-15.376) & (-24.850)\\
%DIFDELCMD < Registers &  &  & 0.022** & 0.049**\\
%DIFDELCMD <  &  &  & (20.517) & (25.969)\\
%DIFDELCMD < Store Type: Dollar Store &  &  & -2.222** & -1.377**\\
%DIFDELCMD <  &  &  & (-46.835) & (-18.903)\\
%DIFDELCMD < Store Type: Convenience Store &  &  & -3.597** & -1.080**\\
%DIFDELCMD <  &  &  & (-31.496) & (-7.747)\\
%DIFDELCMD < Store Type: Other &  &  & -1.451** & -1.331**\\
%DIFDELCMD <  &  &  & (-28.680) & (-19.097)\\
%DIFDELCMD < \midrule
%DIFDELCMD < AIC & 40,811.4 & 35,119.64 & 37,230.81 & 23,556.77\\
%DIFDELCMD < \$\textbackslash{}rho\textasciicircum{}2\_0\$ & 0.179 & 0.293 & 0.251 & 0.526\\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \multicolumn{5}{l}{\rule{0pt}{1em}* p $<$ 0.05, ** p $<$ 0.01}\\
%DIFDELCMD < \multicolumn{5}{l}{\rule{0pt}{1em}t-statistics in parentheses}\\
%DIFDELCMD < \end{tabular}
...
$\rho_0^2$ & 0.179 & 0.293 & 0.251 & 0.526\\
\bottomrule
\multicolumn{5}{l}{\rule{0pt}{1em}* p $<$ 0.05, ** p $<$ 0.01}\\
\multicolumn{5}{l}{\rule{0pt}{1em}t-statistics in parentheses}\\
\end{tabular}
...
%DIFDELCMD < \includegraphics[width=6in,height=\textheight]{04_estimation_files/figure-pdf/fig-map-ut-1.pdf}
%DIFDELCMD < 
...
\includegraphics[width=4in,height=4in]{04_estimation_files/figure-pdf/fig-map-ut-1.pdf}
...
constructed to \DIFdelbegin \DIFdel{ascertain what may be the best strategy to improve
nutrition access in a community}\DIFdelend \DIFaddbegin \DIFadd{illustrate potential applications of a utility-based
model, and not as an evaluation of proposed policies}\DIFaddend . We first describe
how each scenario was constructed, and then discuss the results
together.
...
\DIFaddend \begin{equation}\phantomsection\label{eq-scenu}{
...
\DIFaddend and the total access benefit is defined as the consumer surplus given in
Equation~\ref{eq-cs}. Note that this benefit is denominated in units of
utility (Ben-Akiva \& Lerman, 1985), and the coefficients in
Equation~\ref{eq-scenu} serve to convert between the units of the
variable and utility. Specifically, the \(\beta_{mkt}\) coefficient
represents how many dollars of grocery cost a person is willing to spend
to increase their utility by one unit. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend In actuality, the utility formula
in Equation~\ref{eq-scenu} is a relative utility added by an unknown
constant, \(U_{ij} = f(\mathbf{\beta}, X_{ij}) + C\), but this \(C\)
term is included in all the alternatives and therefore cancels out \DIFaddbegin \DIFadd{in
estimation }\DIFaddend (Train, 2009)\DIFdelbegin \DIFdel{in estimation}\DIFdelend . This means we cannot assess the
\emph{absolute} value of utility, but we can assess the \emph{relative}
monetary benefit of the difference between two scenarios as \[
...
valley community.
\DIFdelbegin \DIFdel{We also implement strategy 2 (an improved store) in
...
availability of quality goods. \DIFaddbegin \DIFadd{In response, some public agencies and
community groups have executed programs to improve the offerings and
display of more nutritive products (Haboush-Deloye et al., 2023; Young
et al., 2018). }\DIFaddend To examine the potential for this strategy to improve
access to nutrition, we improve the attributes of an existing dollar
store in the community so that it has the size, prices, and availability
of goods as a full-service grocery store. As above, we create a
full-service grocery store with a number of registers equal to the mean
of other grocery stores in the community, and NEMS availability score,
NEMS cost score, and market basket cost equivalent to the 75\(^{th}\)
percentile for the community. Thus the store is expected to be
better-than-average quality as perceived by the residents of the
...
dollar store. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The improved stores are at the following locations in each community:
}%DIFDELCMD < 
...
%DIFDELCMD < \item
%DIFDELCMD <   %%%
\DIFdel{An }\DIFdelend \DIFaddbegin \DIFadd{In this scenario, we improve the attributes of an existing
}\DIFaddend ethnic store near 2700 W 3500 S in West Valley City\DIFdelbegin \DIFdel{(Salt Lake)
}%DIFDELCMD < \item
%DIFDELCMD <   %%%
\DIFdel{A small grocery store in Santaquin (Utah County)
}%DIFDELCMD < \item
%DIFDELCMD <   %%%
\DIFdel{A dollar store in Blanding (San Juan County)
...
%DIFDELCMD < \includegraphics[width=6in,height=\textheight]{05_scenarios_files/figure-pdf/fig-s1results-1.pdf}
%DIFDELCMD < 
...
{%DIFAUXCMD
%DIFDELCMD < \label{fig-s1results}%%%
\DIFdelFL{Estimated per-household benefits of adding
new full-service store in west Salt Lake Valley}}
%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \centering{

\includegraphics[width=4in,height=\textheight]{05_scenarios_files/figure-pdf/fig-scenario-results-1.pdf}

}
\DIFaddendFL 

\DIFdelbeginFL %DIFDELCMD < \end{figure}%%%
\DIFdelend \DIFaddbegin \subcaption{\label{fig-scenario-results-1}\DIFadd{New full-service store}}

\end{minipage}\DIFaddend %
%DIF > 
\DIFaddbegin \begin{minipage}{0.50\linewidth}
\DIFaddend 

\DIFdelbegin \DIFdel{Figure~\ref{fig-s1results} }\DIFdelend \DIFaddbegin \centering{

\includegraphics[width=4in,height=\textheight]{05_scenarios_files/figure-pdf/fig-scenario-results-2.pdf}

}

\subcaption{\label{fig-scenario-results-2}\DIFadd{Improved existing store}}
...
\includegraphics[width=4in,height=\textheight]{05_scenarios_files/figure-pdf/fig-scenario-results-3.pdf}

}

\subcaption{\label{fig-scenario-results-3}\DIFadd{Improved non-motorized and
transit access}}
...
\includegraphics[width=4in,height=\textheight]{05_scenarios_files/figure-pdf/fig-scenario-results-4.pdf}

}

\subcaption{\label{fig-scenario-results-4}\DIFadd{Expanded grocery delivery}}

\end{minipage}%DIF > 

\caption{\label{fig-scenario-results}\DIFadd{Estimated per-household benefits
under the four Salt Lake Valley scenarios.}}

\end{sidewaysfigure}%DIF > 

\DIFadd{Figure~\ref{fig-scenario-results} }\DIFaddend shows the geographic distribution of
benefits associated with \DIFaddbegin \DIFadd{each Salt Lake Valley scenario. The benefits
from }\DIFaddend locating a new store at a site in the Salt Lake community
\DIFdelbegin \DIFdel{. The benefits }\DIFdelend \DIFaddbegin \DIFadd{(Figure~\ref{fig-scenario-results-1}) }\DIFaddend are largest immediately next to
the new store, where they exceed \(3\) for each household each time the
household makes a trip to a grocery store. \DIFdelbegin %DIFDELCMD < 
...
%DIFDELCMD < \includegraphics[width=6in,height=\textheight]{05_scenarios_files/figure-pdf/fig-s2results-1.pdf}
%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{fig-s2results}%%%
\DIFdelFL{Estimated per-household benefits of
...
%DIFDELCMD < %%%
\DIFdel{Figure~\ref{fig-s2results} shows the results of the scenario improving
an existing store in the Salt Lake community. Compared to the results of
the new store scenario, }\DIFdelend \DIFaddbegin \DIFadd{The improved-store scenario
(Figure~\ref{fig-scenario-results-2}) is similar in shape, but }\DIFaddend the scale
of the benefits are not as substantial (a maximum per-household-trip
benefit of less than \$1), and seem to not cover quite as large a
geographic region. \DIFdelbegin \DIFdel{Figure~\ref{fig-s2sjut} shows
the results of improving a
store in Utah and San Juan Counties. As in
Salt Lake, }\DIFdelend \DIFaddbegin \DIFadd{The transit and non-motorized access scenario
(Figure~\ref{fig-scenario-results-3}) spreads smaller benefits over a
larger area, while the grocery-delivery scenario
(Figure~\ref{fig-scenario-results-4}) produces the largest and most
broadly distributed gains. In this figures the distribution of benefits
is shown in color, but the reader should note that }\DIFaddend the \DIFdelbegin \DIFdel{benefits are most
strongly concentrated in the immediate
...
\$2, considerably more than the maximum benefit in Salt Lake. This is intuitive, as the improvement of a store matters less if the stores
close to you are already sufficient.
}\DIFdelend \DIFaddbegin \DIFadd{scale of benefits
is not uniform across the scenarios, and is marked instead by the
numeric values on the legend.
}\DIFaddend 
...
%DIFDELCMD < \includegraphics[width=4in,height=\textheight]{05_scenarios_files/figure-pdf/fig-s2sjut-1.pdf}
%DIFDELCMD < 
...
{%DIFAUXCMD
%DIFDELCMD < \label{fig-s2sjut-1}%%%
\DIFdelFL{Utah County (in Santaquin)}}
%DIFAUXCMD
%DIFDELCMD < 
...
%DIFDELCMD < \includegraphics[width=4in,height=\textheight]{05_scenarios_files/figure-pdf/fig-s2sjut-2.pdf}
%DIFDELCMD < 
...
%DIFDELCMD < \subcaption{%
{%DIFAUXCMD
%DIFDELCMD < \label{fig-s2sjut-2}%%%
\DIFdelFL{San Juan County (in Blanding)}}
%DIFAUXCMD
%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < \label{fig-s2sjut}%%%
\DIFdelFL{Estimated per-household benefits of improving
...
%DIFDELCMD < %%%
\DIFdel{The results of the third scenario, improving the access of non-motorized
and public transit access to stores, are shown in
Figure~\ref{fig-s3results}. This benefit is spread over a larger area,
and is concentrated on the
35 MAX bus rapid transit corridor where the
improvement in walk access time to transit couples with high frequency
transit service to large grocery stores on the corridor.
The
...
%DIFDELCMD < \includegraphics[width=6in,height=\textheight]{05_scenarios_files/figure-pdf/fig-s3results-1.pdf}
%DIFDELCMD < 
...
%DIFDELCMD < \includegraphics[width=6in,height=\textheight]{05_scenarios_files/figure-pdf/fig-s4results-1.pdf}
%DIFDELCMD < 
...
%DIFDELCMD < \centering
%DIFDELCMD < \begin{tabular}[t]{lllll}
%DIFDELCMD < \toprule
%DIFDELCMD < \multicolumn{1}{c}{ } & \multicolumn{4}{c}{Weighted by} \\
%DIFDELCMD < \cmidrule(l{3pt}r{3pt}){2-5}
%DIFDELCMD < Scenario & Households & Non-white & Low-Income & Zero-Vehicle\\
%DIFDELCMD < \midrule
%DIFDELCMD < New Store & \$12,860,857 & \$5,612,293 & \$2,217,757 & \$555,185\\
%DIFDELCMD < \addlinespace[0.3em]
%DIFDELCMD < \multicolumn{5}{l}{\textbf{Improved Store}}\\
%DIFDELCMD < \hspace{1em}Salt Lake Valley & \$6,273,501 & \$3,019,389 & \$1,255,096 & \$333,377\\
%DIFDELCMD < \hspace{1em}Utah County & \$8,413,589 & \$1,340,090 & \$1,319,575 & \$163,353\\
%DIFDELCMD < \hspace{1em}San Juan County & \$79,343 & \$35,482 & \$25,767 & \$5,383\\
%DIFDELCMD < Improved Transport & \$1,686,717 & \$767,224 & \$234,676 & \$52,085\\
%DIFDELCMD < Delivery Services & \$228,192,080 & \$88,977,055 & \$26,216,631 & \$5,474,393\\
%DIFDELCMD < \bottomrule
%DIFDELCMD < \end{tabular}
%DIFDELCMD < 
...
Improved Store & \$6,273,501 & \$3,019,389 & \$1,255,096 & \$333,377\\
...
the \DIFdelbegin \DIFdel{three scenarios (and the result of the second scenario in all three
communities). }\DIFdelend \DIFaddbegin \DIFadd{four scenarios }\DIFaddend The Households column multiplies the difference in
...
scenario in the Salt Lake Valley.
\DIFdelbegin \DIFdel{The Utah County store improvement, on
...
magnitude higher than the cost of a single new grocery store. It \DIFdelbegin \DIFdel{should
also be }\DIFdelend \DIFaddbegin \DIFadd{is
}\DIFaddend acknowledged that improving non-automobile infrastructure and services
would have benefits \DIFdelbegin \DIFdel{beyond just }\DIFdelend \DIFaddbegin \DIFadd{for all trip types --- beyond just the }\DIFaddend grocery store
trips \DIFaddbegin \DIFadd{--- }\DIFaddend that we do not attempt to enumerate here. It is also not clear
whether the level of improvement simulated in the second scenario could
be accomplished within the envelope of the existing store; it may be
that such improvements would meet or exceed the cost of building a new
store on a new site, along with stocking, staffing, and operating the
store.
...
achieve profitability later (\DIFaddbegin \DIFadd{D. V. }\DIFaddend Moore, 2022). Resolving all of these
issues cannot be handled in the present analysis, but it must be at
least considered that grocery delivery may have the greatest consumer
surplus of these four strategies by a wide margin.
...
access have perhaps limited efforts at developing solutions. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend In this
research, \DIFdelbegin \DIFdel{the model we developed and informed with
location-based services data suggested that policies that increased the availability and quality of grocery stores would do more to enhance
accessto nutrition than strategies that increased the transit level of service and lowered active transport travel times in three different
Utah communities}\DIFdelend \DIFaddbegin \DIFadd{a model of nutrition resource access that includes attributes
of the store --- especially its size and the price of its goods ---
improves model fit over a model that only includes the transportation
accessibility information. This more sensitive model can be used in
policy evaluations to compare more targeted interventions along varied
dimensions. Travel time proximity is still seen as a driving attribute
of access, explaining most of the variation in choice and leading to a
high evaluation of grocery deliveries that effectively eliminate this
barrier}\DIFaddend . These findings may not be general, but they largely support the
review of food desert literature in \DIFdelbegin \DIFdel{(}\DIFdelend Beaulac et al. \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend 2009) as well as
critical evaluations by \DIFdelbegin \DIFdel{(Shannon , }\DIFdelend \DIFaddbegin \DIFadd{Shannon (}\DIFaddend 2014) and \DIFdelbegin \DIFdel{(}\DIFdelend Wright et al. \DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend 2016).

\DIFaddbegin \DIFadd{The broad literature of attempts to evaluate the nutrition environment
--- in the end --- must be motivated by attempts to improve not merely
access to nutrition resources but to improve health outcomes. To this
end, it would be useful to understand if the more comprehensive measures
pursued in this paper are more strongly correlated with health outcomes
than the less comprehensive measures currently used in policy
evaluations. This has been shown to be the case with urban green spaces
(Macfarlane et al., 2021), but an examination of this effect for health
outcomes is left to future research.
}

\DIFaddend \subsection{Limitations}\label{sec-limitations}
...
the nutrition environment literature. \DIFaddbegin \DIFadd{We acknowledge that NEMS-S
measures may not capture the full range of nutritional needs across
populations and reflect a constrained but standardized proxy for
diet-related healthfulness. }\DIFaddend The NEMS-S focus on low-calorie and low-fat
alternatives may be somewhat outdated in view of modern nutritional
guidelines. For example, the NEMS-S does not track the availability and
price of poultry, as ``lean'' poultry is not a goods category in the way
that ground beef comes with multiple fat contents. Thus a major source
of protein in typical American diets (FNS, 2021) was not traced across
stores in each community. On a more basic level, the NEMS-S attempts to
measure a store's stocking of goods that researchers believe are
beneficial, and does not measure either what people wish to buy, or what
they are actually buying at a store. Future research might attempt to
survey shoppers on what they actually purchased at each store --- or
collect receipts of their purchases --- though this would substantially
raise the difficulty of collecting data.
...
\DIFaddbegin \DIFadd{This research implemented travel impedance measures calculated using an
asserted mode choice utility equation, with assumed limits on the walk
distance and total travel time by all modes. These assumptions are
necessary to render the problem tractable, but more information could be
incorporated in future research. The authors are unaware of travel
behavior data in San Juan County that could be used to estimate mode
choice model parameters, but it is likely that sensitivities to
distances and costs are different in that region than in the urbanized
areas of the Wasatch Front. The limits applied to non-motorized distance
and travel time similarly will affect the numeric results presented in
this paper, but are unlikely to have a significant impact on the
interpretation or the scale of the results. A sensitivity analysis to
these assumptions would be a useful step in implementing this research.
}

\DIFaddend A number of simplifying assumptions concern the socioeconomic and
...
determined by an activity-based model, as proposed by \DIFdelbegin \DIFdel{(}\DIFdelend Dong et al.
\DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend 2006). In this exercise, which we leave to future research, the grocery
...
of \DIFdelbegin \DIFdel{(}\DIFdelend Widener \& Shannon \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend 2014) within the framework of explicit
...
\DIFaddbegin \bibitem[\citeproctext]{ref-ansai2023}
\DIFadd{Ansai, N., Wambogo, E. A., Herrick, K. A., Zimmer, M., Reedy, J., Hales,
C. M., \& Ogden, C. L. (2023). Dietary contributions of food outlets by
urbanization level in the }{\DIFadd{US}} \DIFadd{population aged 2 years and
older---}{\DIFadd{NHANES}} \DIFadd{2013--2018. }\emph{\DIFadd{The American Journal of Clinical
Nutrition}}\DIFadd{, }\emph{117}(5)\DIFadd{, 946--954.
}\url{https://doi.org/10.1016/j.ajcnut.2023.02.014}

\DIFaddend \bibitem[\citeproctext]{ref-modelsummary}
...
\DIFaddbegin \bibitem[\citeproctext]{ref-baker2024}
\DIFadd{Baker, K., Burd, L., \& Figueroa, R. (2024). Consumer nutrition
environment measurements for nutrient-dense food availability and food
sustainability: A scoping review. }\emph{\DIFadd{Archives of Public Health}}\DIFadd{,
}\emph{82}(1)\DIFadd{, 7. }\url{https://doi.org/10.1186/s13690-023-01231-y}

\DIFaddend \bibitem[\citeproctext]{ref-barnes2021}
...
\DIFaddbegin \bibitem[\citeproctext]{ref-bennion2022}
\DIFadd{Bennion, N., Redelfs, A. H., Spruance, L., Benally, S., \& Sloan-Aagard,
C. (2022). Driving }{\DIFadd{Distance}} \DIFadd{and }{\DIFadd{Food Accessibility}}\DIFadd{: }{\DIFadd{A Geospatial
Analysis}} \DIFadd{of the }{\DIFadd{Food Environment}} \DIFadd{in the }{\DIFadd{Navajo Nation}} \DIFadd{and }{\DIFadd{Border
Towns}}\DIFadd{. }\emph{\DIFadd{Frontiers in Nutrition}}\DIFadd{, }\emph{\DIFadd{9}}\DIFadd{, 904119.
}\url{https://doi.org/10.3389/fnut.2022.904119}

\DIFaddend \bibitem[\citeproctext]{ref-bureauoflaborstatistics2023}
...
\DIFaddbegin \bibitem[\citeproctext]{ref-burns2007}
\DIFadd{Burns, C. M., \& Inglis, A. D. (2007). Measuring food access in
}{\DIFadd{Melbourne}}\DIFadd{: }{\DIFadd{Access}} \DIFadd{to healthy and fast foods by car, bus and foot in
an urban municipality in }{\DIFadd{Melbourne}}\DIFadd{. }\emph{\DIFadd{Health \& Place}}\DIFadd{,
}\emph{13}(4)\DIFadd{, 877--885.
}\url{https://doi.org/10.1016/j.healthplace.2007.02.005}

\bibitem[\citeproctext]{ref-cetateanu2016}
\DIFadd{Cetateanu, A., Luca, B.-A., Popescu, A. A., Page, A., Cooper, A., \&
Jones, A. (2016). A novel methodology for identifying environmental
exposures using }{\DIFadd{GPS}} \DIFadd{data. }\emph{\DIFadd{International Journal of Geographical
Information Science}}\DIFadd{, }\emph{30}(10)\DIFadd{, 1944--1960.
}\url{https://doi.org/10.1080/13658816.2016.1145682}

\bibitem[\citeproctext]{ref-charreire2010}
\DIFadd{Charreire, H., Casey, R., Salze, P., Simon, C., Chaix, B., Banos, A.,
Badariotti, D., Weber, C., \& Oppert, J.-M. (2010). Measuring the food
environment using geographical information systems: A methodological
review. }\emph{\DIFadd{Public Health Nutrition}}\DIFadd{, }\emph{13}(11)\DIFadd{, 1773--1785.
}\url{https://doi.org/10.1017/S1368980010000753}

\DIFaddend \bibitem[\citeproctext]{ref-chen2016}
...
\DIFaddbegin \bibitem[\citeproctext]{ref-economicresearchservice2025}
\DIFadd{Economic Research Service. (2025). }\emph{\DIFadd{Food }{\DIFadd{Access Research Atlas}} \DIFadd{-
}{\DIFadd{Documentation}}}\DIFadd{.
https://www.ers.usda.gov/data-products/food-access-research-atlas/documentation.
}

\DIFaddend \bibitem[\citeproctext]{ref-fitzpatrick2006}
...
\DIFaddbegin \bibitem[\citeproctext]{ref-glanz2023}
\DIFadd{Glanz, K., Fultz, A. K., Sallis, J. F., Clawson, M., McLaughlin, K. C.,
Green, S., \& Saelens, B. E. (2023). Use of the }{\DIFadd{Nutrition Environment
Measures Survey}}\DIFadd{: }{\DIFadd{A Systematic Review}}\DIFadd{. }\emph{\DIFadd{American Journal of
Preventive Medicine}}\DIFadd{, }\emph{65}(1)\DIFadd{, 131--142.
}\url{https://doi.org/10.1016/j.amepre.2023.02.008}

\DIFaddend \bibitem[\citeproctext]{ref-glanz2007}
...
\DIFaddbegin \bibitem[\citeproctext]{ref-haboush2023healthy}
\DIFadd{Haboush-Deloye, A. L., Knight, M. A., Bungum, N., \& Spendlove, S.
(2023). Healthy foods in convenience stores: Benefits, barriers, and
best practices. }\emph{\DIFadd{Health Promotion Practice}}\DIFadd{, }\emph{\DIFadd{24}}\DIFadd{(1\_suppl),
108S--111S. }\url{https://doi.org/10.1177/15248399221147878}

\DIFaddend \bibitem[\citeproctext]{ref-handy1997}
...
\DIFaddbegin \bibitem[\citeproctext]{ref-jaskiewicz2016}
\DIFadd{Jaskiewicz, L., Block, D., \& Chavez, N. (2016). Finding }{\DIFadd{Food Deserts}}\DIFadd{:
}{\DIFadd{A Comparison}} \DIFadd{of }{\DIFadd{Methods Measuring Spatial Access}} \DIFadd{to }{\DIFadd{Food Stores}}\DIFadd{.
}\emph{\DIFadd{Health Promotion Practice}}\DIFadd{, }\emph{17}(3)\DIFadd{, 400--407.
}\url{https://www.jstor.org/stable/26734031}

\DIFaddend \bibitem[\citeproctext]{ref-kaczynski2016}
...
\DIFaddbegin \bibitem[\citeproctext]{ref-kostopoulou2024}
\DIFadd{Kostopoulou, E., Christofa, E., Gonzales, E., \& Krevat, D. (2024).
Measuring and modeling food accessibility by transportation mode.
}\emph{\DIFadd{Journal of Transport Geography}}\DIFadd{, }\emph{\DIFadd{118}}\DIFadd{, 103907.
}\url{https://doi.org/10.1016/j.jtrangeo.2024.103907}

\DIFaddend \bibitem[\citeproctext]{ref-liu2022}
...
\DIFaddbegin \bibitem[\citeproctext]{ref-macfarlane2021modeling}
\DIFadd{Macfarlane, G. S., Boyd, N., Taylor, J. E., \& Watkins, K. (2021).
Modeling the impacts of park access on health outcomes: A utility-based
accessibility approach. }\emph{\DIFadd{Environment and Planning B: Urban
Analytics and City Science}}\DIFadd{, }\emph{48}(8)\DIFadd{, 2289--2306.
}\url{https://doi.org/10.1177/2399808320974027}

\DIFaddend \bibitem[\citeproctext]{ref-macfarlane2022}
...
\DIFaddbegin \bibitem[\citeproctext]{ref-moore2008}
\DIFadd{Moore, L. V., Diez Roux, A. V., \& Brines, S. (2008). Comparing
}{\DIFadd{Perception-Based}} \DIFadd{and }{\DIFadd{Geographic Information System}} \DIFadd{(}{\DIFadd{GIS}}\DIFadd{)-}{\DIFadd{Based
Characterizations}} \DIFadd{of the }{\DIFadd{Local Food Environment}}\DIFadd{. }\emph{\DIFadd{Journal of
Urban Health}}\DIFadd{, }\emph{85}(2)\DIFadd{, 206--216.
}\url{https://doi.org/10.1007/s11524-008-9259-x}

\bibitem[\citeproctext]{ref-murphy2017}
\DIFadd{Murphy, M., Koohsari, M. J., Badland, H., \& Giles-Corti, B. (2017).
Supermarket access, transport mode and }{\DIFadd{BMI}}\DIFadd{: The potential for urban
design and planning policy across socio-economic areas. }\emph{\DIFadd{Public
Health Nutrition}}\DIFadd{, }\emph{20}(18)\DIFadd{, 3304--3315.
}\url{https://doi.org/10.1017/S1368980017002336}

\bibitem[\citeproctext]{ref-park2025}
\DIFadd{Park, Y., Na, J., Jang, K., Lee, D., Kwon, C., \& Yoon, Y. (2025).
}\emph{\DIFadd{Food }{\DIFadd{Deserts}} \DIFadd{in }{\DIFadd{Context}}\DIFadd{: }{\DIFadd{How Site-Specific Factors Reshape
Food Deserts Discussion}}} \DIFadd{(\{\{SSRN Scholarly Paper\}\} 5632970). Social
Science Research Network. }\url{https://doi.org/10.2139/ssrn.5632970}

\DIFaddend \bibitem[\citeproctext]{ref-pereira2021}
...
\DIFaddbegin \bibitem[\citeproctext]{ref-rhone2017}
\DIFadd{Rhone, A., Ver Ploeg, M., Dicken, C., Williams, R., \& Breneman, V.
(2017). }\emph{\DIFadd{Low-}{\DIFadd{Income}} \DIFadd{and }{\DIFadd{Low-Supermarket-Access Census Tracts}}\DIFadd{,
2010-2015}} \DIFadd{(Economic }{\DIFadd{Information Bulletin Number}} \DIFadd{165).
}\url{https://doi.org/10.22004/ag.econ.262134}

\DIFaddend \bibitem[\citeproctext]{ref-shannon2014}
...
\DIFaddbegin \bibitem[\citeproctext]{ref-widener2018}
\DIFadd{Widener, M. J., Minaker, L. M., Reid, J. L., Patterson, Z., Ahmadi, T.
K., \& Hammond, D. (2018). Activity space-based measures of the food
environment and their relationships to food purchasing behaviours for
young urban adults in }{\DIFadd{Canada}}\DIFadd{. }\emph{\DIFadd{Public Health Nutrition}}\DIFadd{,
}\emph{21}(11)\DIFadd{, 2103--2116.
}\url{https://doi.org/10.1017/S1368980018000435}

\DIFaddend \bibitem[\citeproctext]{ref-widener2014}
...
\bibitem[\citeproctext]{ref-young2018}
\DIFadd{Young, S., DeNomie, M., Sabir, J., Gass, E., \& Tobin, J. (2018). Around
the corner to better health: A milwaukee corner store initiative.
}\emph{\DIFadd{American Journal of Health Promotion}}\DIFadd{, }\emph{32}(6)\DIFadd{, 1353--1356.
}\url{https://doi.org/10.1177/0890117117736970}
\DIFaddend 
