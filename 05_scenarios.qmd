...
```{r scenario-table-data, include=FALSE}
ut_base <- tar_read(wf_access)
sj_base <- tar_read(ru_access)
s2_altd_ut <- tar_read(s2_access_ut)
s2_altd_sj <- tar_read(s2_access_sj)

s2cost_ut <- bg |>  select(id = GEOID) |> filter(substr(id, 3, 5) == "049") |>
  left_join(bg_acs, by = c("id" = "geoid")) |>
  sf::st_transform(4326) |>
  left_join(ut_base |> rename(base = dclogsum)) |>
  left_join(s2_altd_ut |> rename(alt = dclogsum)) |>
  mutate(diff = (base - alt) / mkt_betas["Utah.market"])

s2cost_sj <- bg |>  select(id = GEOID) |> filter(substr(id, 3, 5) == "037") |>
  left_join(bg_acs, by = c("id" = "geoid")) |>
  sf::st_transform(4326) |>
  left_join(sj_base |> rename(base = dclogsum)) |>
  left_join(s2_altd_sj |> rename(alt = dclogsum)) |>
