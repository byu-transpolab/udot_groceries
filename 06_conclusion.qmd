...
In this research, a model of nutrition resource access that includes attributes of
the store --- especially its size and the price of its goods --- improves model fit 
over a model that only includes the transportation accessibility information. This more
sensitive model can be used in policy evaluations to compare more targeted 
interventions along varied dimensions. Travel time proximity is still seen as a driving 
attribute of access, explaining most of the variation in choice and leading to a
high evaluation of grocery deliveries that effectively eliminate this barrier.
These findings may not be general, but they largely
support the review of food desert literature in @beaulac2009 as well as 
critical evaluations by @shannon2014 and @wright2016.

The broad literature of attempts to evaluate the nutrition environment --- in the end ---
must be motivated by attempts to improve not merely access to nutrition resources 
but to improve health outcomes. To this end, it would be useful to understand if 
the more comprehensive measures pursued in this paper are more strongly correlated
with health outcomes than the less comprehensive measures currently used in 
policy evaluations. This has been shown to be the case with urban green spaces
[@macfarlane2021modeling], but an examination of this effect for
health outcomes is left to future research.
...
literature. We acknowledge that NEMS-S measures may not capture the full range
of nutritional needs across populations and reflect a constrained but standardized
proxy for diet-related healthfulness. 
The NEMS-S focus on low-calorie and low-fat alternatives may be
...
This research implemented travel impedance measures calculated using an asserted 
mode choice utility equation, with assumed limits on the walk distance and total travel time 
by all modes. These assumptions are necessary to render the problem tractable, but 
more information could be incorporated in future research. The authors are unaware of
travel behavior data in San Juan County that could be used to estimate mode choice
model parameters, but it is likely that sensitivities to distances and costs are different
in that region than in the urbanized areas of the Wasatch Front. The limits applied to
non-motorized distance and travel time similarly will affect the numeric results presented
in this paper, but are unlikely to have a significant impact on the interpretation or the scale
of the results. A sensitivity analysis to these assumptions would be a useful
