...
```{r map-legend-theme, include = FALSE, cache = FALSE}
map_legend_theme <- theme(
...
map_legend_theme_wide <- theme(
  legend.title = element_text(size = 6),
  legend.text = element_text(size = 5.5),
  legend.key.height = grid::unit(0.10, "in"),
  legend.key.width = grid::unit(0.08, "in")
)
```
...
ggplot(nems_groceries, aes(x = availability, y = market, group = county)) + 
  ...
  xlab("NEMS Availability Score")  +
  ylab("Market Basket Cost [USD]") +
...
gm <- list(
  ...
  list("raw" = "rho20", "clean" = "$\\rho_0^2$", "fmt" = f))
...
modelsummary(ut_models, gof_map = gm, coef_map = cm, 
             stars = c('*' = 0.05, '**' = 0.01),
             statistic = "({statistic})",
             notes = list("t-statistics in parentheses"), escape = FALSE,
             output="kableExtra")
```
