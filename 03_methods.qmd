...
map_regions <- bg_regions |> left_join(grocery_regions, by = "county") |> 
  mutate(zoom_level = c(11, 12, 8))

p <- lapply(seq_along(1:nrow(map_regions)), function(i){
  bbox <- st_bbox(map_regions$grocery[[i]])
  ggplot() +
    ...
    coord_sf(xlim = bbox[c(1,3)], ylim = bbox[c(2,4)], crs = st_crs(map_regions$grocery[[i]])) +
