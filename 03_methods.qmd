...
West Valley City. The housing density and locations of stores in each community are shown in @fig-communities,
and the context of each community in Utah is shown in @fig-community-context.


```{r}
#| label: fig-communities
#| fig-cap: Location of surveyed stores in three comunities in Utah, with housing density at block group level and size of store measured by number of registers.
#| fig-env: "sidewaysfigure"
#| fig-width: 9
#| fig-height: 6
#| dev: "tikz"
tar_load(map_regions)
library(patchwork)
p <- lapply(seq_along(1:nrow(map_regions)), function(i){
  bbox <- sf::st_bbox(map_regions$grocery[[i]])
  ggplot() +
    ggspatial::annotation_map_tile("cartolight", zoom = map_regions$zoom_level[i]) +
    ggspatial::annotation_scale(location = "tr") +
    ggspatial::annotation_north_arrow(style =  ggspatial::north_arrow_minimal, 
      location = "tr", pad_y = unit(0.5, "in")) +
    ggtitle(map_regions$county[i]) +
    ...
    geom_sf(data = map_regions$region[[i]], alpha = 0.3, mapping = aes(fill = density), lwd = 0.01) +
    geom_sf(data = map_regions$grocery[[i]], mapping = aes(size = total_registers), color = "grey40", alpha = 0.7) +
    coord_sf(xlim = bbox[c(1,3)], ylim = bbox[c(2,4)], crs = sf::st_crs(map_regions$grocery[[i]])) +
    scale_fill_viridis_c("Density [Housing Units / sqkm] ", direction = -1, 
      limits = c(0, 2000), oob = scales::squish,
      breaks = c(0, 100, 500, 1000, 2000), transform = "sqrt", na.value = NA, option = "A") +
    scale_size_manual("Stores: Total Registers", values = c("<5" = 1, "5-10" = 2, "10-25" = 3, ">25" = 4), na.value = NA,
      labels = c("\\textless 5", "5-10", "10-25", "\\textgreater 25"))
})
...
patch <- (p1 | (p2/p3)) + plot_layout(guides = "collect", widths =  c(1, 1))
patch
```

```{r message=FALSE}
#| label: fig-community-context
#| fig-cap: Context of communities in @fig-communities within the state of Utah.
#| 
bbox_polygon <- function(grocery) {
  bbox <- sf::st_bbox(grocery)
...
community_context_box_geoms <- map_regions$grocery |>
  purrr::map(bbox_polygon) |>
  do.call(what = sf::st_sfc, args = _) |>
  sf::st_set_crs(sf::st_crs(map_regions$grocery[[1]]))

regions <- sf::st_sf(
  county = map_regions$county,
  geometry = community_context_box_geoms
)
utah <- tigris::states(progress_bar = FALSE) |> filter(STUSPS == "UT")
...
ggplot() +
  ...
  ggspatial::annotation_north_arrow(style =  ggspatial::north_arrow_minimal,
        location = "tr", pad_y = unit(0.5, "in")) +
  geom_sf(data = utah, fill = NA, color = "black", alpha = 0.5) +
  geom_sf(data = regions, fill = "white", color = "red", alpha = 0.5) +
  theme(
    legend.title = element_text(size = rel(1)),
    legend.text = element_text(size = rel(0.8)),
    plot.title = element_text(size = rel(1.2)),
...
significant differences in the availability and cost of foods identified as 
more healthful within the NEMS-S framework, and has been validated for this purpose. 
Beyond superficial
...
Within the NEMS-S framework, "healthful" options are operationalized primarily
through the availability and relative pricing of lower-fat and lower-calorie
alternatives;
NEMS-S captures one dimension of diet-related healthfulness but does not
represent a comprehensive assessment of nutritional quality across all
populations.
...
as well as NEMS-S defined more healthful options; and a cost score describing the spread between
...
in the store as defined by the NEEMS-S framework (i.e., low-calorie chips or low-fat milk).
...
healthful and less healthful options as defined by the NEMS-S framework: if the price of whole wheat bread is cheaper
...
advantageous pricing scheme towards those options. Accordingly, higher scores
reflect greater availability and more favorable pricing of these NEMS-S defined
healthful options, rather than a comprehensive measure of overall nutritional
quality.
...
amount of poultry, but the NEMS-S instrument collects the per-pound cost of ground
...
determine each store's location, brand name, and store type, information which we also
...
independent imputations.  When using the imputed data for analysis, we take the
mean of the ten predictions for continuous values, and the mode for discrete
values.
...
comparative tradeoffs between transportation-focused and nutrition environment-focused
improvements. It is therefore essential that the measure of travel impedance incorporate 
travel by multiple modes of transportation, so that highway improvements and transit or
active transport improvements can be compared.
...
utility of multiple destination attributes and costs, the
log-sum of a mode choice model is the expected maximum utility of all available travel modes, 
weighted by preferences and costs [@ben-akiva1985]. In this study we assert the following mode choice utility equations:
\begin{align*} 
...
2023; this period on a Tuesday morning in the AM peak provides the highest level of
service possible in the transit system. The total allowable trip time by any mode was set to 120 minutes, and the
walk distance was capped at 10 kilometers, mirroring limits set in the regional travel model. 
If a particular $i,j$ pair exceeded
